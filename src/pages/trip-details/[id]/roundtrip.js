...
export default function RoundTrip() {
  ...
  }, [router.isReady, departId, returnId]);
          ...
          <Paper elevation={3} className={`${classes.mainPaper}`}>
            <Box className={`${classes.paperUpper}`}>
              <Box className={`${classes.flightMain}`}>
                ...
                <Box className={`${classes.flightBox}`}>
                  <Typography variant="h5">
                    {departTickets.length
                      ? formatDate(departTickets[0].flight.departureDay)
                      : ""}
                  </Typography>
                  <Box className={`${classes.flightDetail}`}>
                    <Typography variant="h5" fontWeight="700">
                      {depart.departureAirport?.airport_code} to{" "}
                      {arrive.arrivalAirport?.airport_code}
                    </Typography>
                    <Typography>
                      {convertTime(depart.departureTime)} -{" "}
                      {convertTime(arrive.arrivalTime)}
                    </Typography>
                  </Box>
                  {departTickets.length > 1 ? (
                    ...
                    >
                      <Typography fontWeight="700">Nonstop</Typography>
                    </Card>
                  )}
                </Box>
                ...
                <Box className={`${classes.returnFlightBox}`}>
                  <Typography variant="h5">
                    {returnTickets.length
                      ? formatDate(returnTickets[0].flight.departureDay)
                      : ""}
                  </Typography>
                  <Box className={`${classes.flightDetail}`}>
                    <Typography variant="h5" fontWeight="700">
                      {returnDepart.departureAirport?.airport_code} to{" "}
                      {returnArrive.arrivalAirport?.airport_code}
                    </Typography>
                    <Typography>
                      {convertTime(returnDepart.departureTime)} -{" "}
                      {convertTime(returnArrive.arrivalTime)}
                    </Typography>
                  </Box>
              ...
              <Card className={`${classes.pricingMain}`}>
                ...
                <Box className={`${classes.price}`}>
                  <Typography>Taxes and Fees per passenger</Typography>
                  <Typography>
                ...
                <Box className={`${classes.price}`}>
                  <Typography>Total per passenger</Typography>
                  <Typography>
                    ${(totalPrice + fees + totalPrice * taxes).toFixed(2)}
                  </Typography>
                </Box>
                <Box className={`${classes.price}`}>
                  <Typography>Numbers of passengers</Typography>
